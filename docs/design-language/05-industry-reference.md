...
- **A newsletter capture with a field and a button.** Pholio's equivalent of an
  instruction is already present as a single word, `Get scouted`, which is the
  industry's own verb (see §4.3).
...
### 4.3 `Get Scouted` is the industry's verb

IMG, Ford and Next Management all use the exact string `Get Scouted` as their
talent-facing action. It appears in IMG's top navigation, in Ford's overlay, and
as one of the two exits from Next's fraud interstitial.

Pholio's footer already uses `Get scouted` for the signed-out account link. That
is the correct word and it should not be replaced with a generic
